 ...
 * 
 * @version $Id: GPSLoggerServiceManager.java 905 2011-03-22 20:06:20Z rcgroot $
...
public class GPSLoggerServiceManager
{
   ...
   public int getLastSignalStrength()
   {
      synchronized (mStartLock)
      {
         int lastSignalStrength = 99;
         try
         {
            if( mBound )
            {
               lastSignalStrength = this.mGPSLoggerRemote.getLastSignalStrength();
            }
            else
            {
               Log.w( TAG, "Remote interface to logging service not found. Started: " + mBound );
         ...
         {
            Log.e( TAG, "Could get lastSignalStrength GPSLoggerService.", e );
         }
